...
def parse_arguments():
    ...
    parser.add_argument("--Realtime", action='store_false', help="Whether to use realtime mode. Default is false.") # store_true -> sets game speed fast
...
def run():
    ...
    if args.LadderServer:
        ...
        #run_game(sc2.maps.get(args.Map),[bot, Computer(Race[args.ComputerRace], Difficulty[args.ComputerDifficulty])],realtime=args.Realtime,sc2_version=args.Sc2Version, )

        run_game(sc2.maps.get(args.Map), [ Bot(Race.Zerg, WorkerRushBot()), Computer(Race.Protoss, Difficulty.Medium) ], realtime=True)
# Start game
